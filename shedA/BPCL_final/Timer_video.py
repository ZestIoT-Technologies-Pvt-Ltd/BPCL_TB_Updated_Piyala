...
config1="/home/zestiot/BPCL_4/BPCL_final/UI_parameters.json"
config="/home/zestiot/BPCL_4/BPCL_final/BPCL_config.json"
...
def event_call(event,temp,path):
	...
	try:
		sc=ClientSocket(device_id=str('BPCL_PYL_NX_0005'))
		...
		if algo == "direction":
			...
			if Dflag ==True:
				...
				if Dcheck == 0 and Dtimer == 0:
					...
					Dcheck = 1
				if datetime.now() > Dst_time + timedelta(seconds=3) and Dtimer == 0 and Dcheck == 1 and Ptimer == 0:
					flag = False
					if Mtimer == 1:
						Mtrigger =0
						Mtimer=0
