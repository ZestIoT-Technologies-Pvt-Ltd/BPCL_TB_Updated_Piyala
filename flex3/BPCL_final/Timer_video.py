...
config1="/home/zestiot/BPCL_2/BPCL_final/UI_parameters.json"
config="/home/zestiot/BPCL_2/BPCL_final/BPCL_config.json"
...
def start_video(event):
	...
	vid_name="DESKTOP_BATH1_BPCL_NX3_"+description+"_"+event+"_"+(datetime.now()).strftime("%Y-%m-%dT%H-%M-%S")+".avi"
	if event == "EVENT21_ON":
		Ppath = loc+vid_name
...
def video_function(event,cam):
	global Pvend_time,Dvend_time,Mvend_time,Ptimer,Dtimer,Mtimer,Pevent_out,Devent_out,Mevent_out,move_flag
	while True:
		if (Dtimer > 0 or Mtimer > 0 or Ptimer >0) and move_flag == True:
			#print("Move value - > "+ str(move_flag))
			img = cam.get_frame()
			#print (img)
...
def event_call(event,temp,path):
	...
	try:
		sc=ClientSocket(device_id=str('BPCL_PYL_NX_0003'))
			...
			if Dflag ==True:
				...
				if Dtimer != 0 and Ddetect > Dalert_frame:
					if Dtimer ==1:
						...
						#Dtimer=0
					elif Dback ==1 and datetime.now() > Drectify +timedelta(seconds=attentive_rectify) and Ptimer == 0:
						with open(event_file,'w') as efile:
		...
		if algo == "motion":
			...
			if Mflag == True:
				...
				if Mcheck == 0 and Mtimer == 0:
					...
					Mcheck = 1
				if datetime.now() > Mst_time + timedelta(seconds=3) and Mtimer == 0 and Mcheck == 1 and Dtimer != 2:
					flag = False
					Mtimer=1
					Mback=0
				...
				elif Mtimer ==1 and Mdetect <= Malert_frame and datetime.now() > Mtrigger + timedelta(seconds=180):
					with open(event_file,'w') as efile:
						#efile.write("EVENT23_OFF",(datetime.now()).strftime("%Y_%m_%dT%H-%M-%S"))
...
def continue_event(off_time):
	global Mtrigger,Dtrigger,Ptrigger,Mvend_time,Pvend_time,Dvend_time
	print("Initial time P -> {}  D -> {}  M -> {}".format(Ptrigger,Dtrigger,Mtrigger))
	print("Initial video_Time  Pv -> {}  Dv -> {}  Mv -> {}".format(Pvend_time,Dvend_time,Mvend_time))
	if Ptrigger != 0:
		Ptrigger = Ptrigger + timedelta(seconds = off_time)
	if Pvend_time !=0:
		Pvend_time = Pvend_time + timedelta(seconds = off_time)
	if Dtrigger != 0:
		Dtrigger = Dtrigger + timedelta(seconds = off_time)
	if Dvend_time !=0:
		Dvend_time = Dvend_time + timedelta(seconds = off_time)
	if Mtrigger != 0:
		Mtrigger = Mtrigger + timedelta(seconds = off_time)
	if Mvend_time !=0:
		Mvend_time = Mvend_time + timedelta(seconds = off_time)
	print("After Off_Time time P -> {}  D -> {}  M -> {}".format(Ptrigger, Dtrigger,Mtrigger))
	print("After video_Time time Pv -> {}  Dv -> {}  Mv -> {}".format(Pvend_time,Dvend_time,Mvend_time))
